...
# pacchetti installati precedentemente:
library(raster)
library(RStoolbox)
...
sent <- brick("sentinel.png") # brick import all the data of satellite image. the extantion is grd file, graphicic file
...
ndvi <- (nir-red) / (nir+red) # Calculation of the Normalized Difference Vegetation Index (NDVI)
plot(ndvi)
cl <- colorRampPalette(c('black','white','red','magenta','green'))(100) # 
plot(ndvi,col=cl)

ndvisd3 <- focal(ndvi, w=matrix(1/9, nrow=3, ncol=3), fun=sd) # window
